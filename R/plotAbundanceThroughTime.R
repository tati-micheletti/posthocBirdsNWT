plotAbundanceThroughTime <- function(pixelsSummaries, # Where the mean rasters are (effectsRasters)!
  ...
  if (useFuture) plan("multiprocess", workers = 4) # Hard coded as it is very mem demanding! 190Gb per species!
  plotMaps <- future_lapply(names(pixelsSummaries), FUN = function(sp){
    tic(paste0("Total Time elapsed for ", sp))
    tb <- qs::qread(pixelsSummaries[[sp]])
    factorialRasters <- lapply(names(comparisons), FUN = function(eachComparison){
      tic(paste0("Time elapsed for ", sp, " for ", eachComparison))
      individualPlotTablePath <- file.path(pathOutputs, paste("timePlotTable", eachComparison, 
                                                              sp, sep = "_"))
      statsEachComparisonPath <- file.path(pathOutputs, paste("timePlotStats", eachComparison, 
                                                              sp, sep = "_"))
      if (any(!file.exists(paste0(individualPlotTablePath, ".qs")),
        ...
        allComparisons <- unique(allComparisons$allComps)
        ...
        gc()
        return(list(individualPlotTable = paste0(individualPlotTablePath, ".qs"),
                    statsEachComparison = paste0(statsEachComparisonPath, ".qs")))
      } else {
        return(list(individualPlotTable = paste0(individualPlotTablePath, ".qs"),
                    statsEachComparison = paste0(statsEachComparisonPath, ".qs")))
      }
